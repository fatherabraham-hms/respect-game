...
import { zVoteWithProp } from "ortypes/orclient.js";
import { TxFailed } from "./errors.js";
import { ORContext as ORContextOrig, } from "ortypes/orContext.js";
...
import { ProposalNotCreated, zPropId, zBytes, encodeVoteMemo } from "ortypes";
import { sleep, stringify } from "ts-utils";
import { z } from "zod";
// Re-define so that ORContext docs are included
export class ORContext extends ORContextOrig {
}
function isPropCreated(propState) {
...
export class ORClient {
    constructor(context, cfg = defaultConfig) {
        this._ctx = context;
    ...
    async getProposals(spec) {
        ...
        for (const nprop of nprops) {
            ...
            const passStageFilter = spec?.stageFilter === undefined ||
                spec.stageFilter.includes(tprop.stage);
            const passVoteStatFilter = spec?.voteStatFilter === undefined ||
     ...
     */
    async proposeBreakoutResult(request, vote = { vote: "Yes" }) {
     ...
     */
    async proposeRespectTo(req, vote = { vote: "Yes" }) {
        const v = zVoteWithProp.parse(vote);
        console.debug("parsed vote");
     ...
     * );
     */
    async proposeBurnRespect(req, vote = { vote: "Yes" }) {
        const v = zVoteWithProp.parse(vote);
     ...
     */
    async proposeCustomSignal(req, vote = { vote: "Yes" }) {
        const v = zVoteWithProp.parse(vote);
     ...
     * await c.proposeTick();
     */
    async proposeTick(req = {}, vote = { vote: "Yes" }) {
        const v = zVoteWithProp.parse(vote);
     ...
     * If you want to make this proposal but don't want to vote for it, specify `{ vote: "None" }`.
     */
    async proposeCustomCall(req, vote = { vote: "Yes" }) {
        const v = zVoteWithProp.parse(vote);
    ...
    async getMaxLiveYesVotes() {
        return Number(await this._ctx.orec.maxLiveYesVotes());
    }
    async _submitProposal(proposal, vote) {
    ...
    async _submitPropToOrnode(proposal) {
        let attempts = 0;
        let _err;
    ...
    async _submitPropToChain(proposal, vote) {
    ...
    async _submitPropTx(proposal, vote) {
        if (vote !== undefined && vote.vote !== "None") {
            const v = this._clientToNode.transformVoteType(vote.vote);
            return this._ctx.orec.vote(proposal.id, v, encodeVoteMemo(vote.memo));
        }
        else {
            return this._ctx.orec.propose(proposal.id);
        }
    }
    async _handleTxPromise(promise, confirms, errMsg) {
        let resp;
        let receipt;
        try {
            resp = await promise;
